...
func parseInput(v any) (any, error) {
...
func ParseInputs(inputsJSON []byte) (map[string]any, error) {
	inputsRAW := make(map[string]any)
...
func _flatSlice(acc *[]*big.Int, v any) {
	rv := reflect.ValueOf(v)
	switch rv.Kind() {
	case reflect.Slice:
		for i := 0; i < rv.Len(); i++ {
