...
var (
	...
	cz   = 𐌈(z0, z0.H(), sys(one, 1)).Add(1, 𐌈(z1, z1.H(), sys(σz, 1)))
	cnot = 𐌈(z0, z0.H(), sys(one, 1)).Add(1, 𐌈(z1, z1.H(), sys(σx, 1)))
)
...
func T1(s []complex64) *Dense {
	return &Dense{Axis: []Axis{{System: 0, Braket: Ket}}, D: tensor.T1(s)}
}

func T2(s [][]complex64) *Dense {
	return &Dense{Axis: []Axis{{System: 0, Braket: Ket}, {System: 0, Braket: Bra}}, D: tensor.T2(s)}
}
...
func (a *Dense) H() *Dense {
	if len(a.Axis) == 1 {
		ax := a.Axis[0]
		ax.Braket = (ax.Braket + 1) % 2
		return &Dense{Axis: []Axis{ax}, D: a.D.Conj()}
	}
